...
async function getStreams() {
  try {
    const response = await axios.post(
        "https://www.classicalcalifornia.org/api/graphql-proxy",
        streamsQuery,
    );
    const streams = response.data.data.streamCollection.items.map(
        (stream) => ({
          id : stream.sys.id,
          name : stream.name,
...
async function getStreamInfo(callSign) {
  ...
  data = data.reduce((acc, mp) => {
    ...
    var tmp = {
      ...
      // container: mp.format,
      bitrate : mp.bitrate,
      url : server.ports.port.type + "://" + server.ip + ":" +
                server.ports.port["#text"],
    }; // store copy of mount mountpoint
    if (tmp.codec.includes("aac"))
      tmp.codec = "aac";
    else if (tmp.codec.includes("mp3"))
      tmp.codec = "mp3";
    acc[mp.mount] = tmp;
    return acc;
  }, {});
...
async function getStreamUrl(streamId, preferredStream = "AAC96") {
  ...
  var stream = {};
  if (streamInfo
          .filter((mp) => mp.mount.replace(streamId, "") == preferredStream)
          .length > 0)
  ...
  // find the preferred stream
    ...
    stream =
        streamInfo[0]; // default to first stream if preferred stream not found
  }
...
async function getCurrentMetadata(streamId, includeImage = false) {
  var response = await axios.get(
      `https://schedule.kusc.org/v3/songs/${streamId}/now?includeImage=${
          includeImage}`,
  );
  ...
  info = {
    ...
    duration : new Date(info.end.dateTime).getTime() -
  ...
  if (includeImage)
    info.image = info.extraInfo.image;
  return info;
}
